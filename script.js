...
function formatDate(value) {
  return value
    ? new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date(`${value}T12:00:00`))
    : 'a day that feels right';
}

function updatePreview() {
  const time = timeInput.value ? ` at ${timeInput.value}` : '';
  const vibe = noteInput.value.trim();
  previewText.textContent = vibe
    ? `${formatDate(dateInput.value)}${time} — ${vibe}. Sounds perfect.`
    : `${formatDate(dateInput.value)}${time} is waiting for us.`;
}

function moveNoButton() {
  const x = Math.random() * Math.max(actionPanel.clientWidth - noBtn.offsetWidth, 0);
  const y = Math.random() * Math.max(actionPanel.clientHeight - noBtn.offsetHeight, 0);
  Object.assign(noBtn.style, { left: `${x}px`, top: `${y}px`, right: 'auto', bottom: 'auto' });
}

function resetNoButtonPosition() {
  Object.assign(noBtn.style, { left: 'auto', top: 'auto', right: '0', bottom: '0' });
}
...
vibeChips.forEach((chip) => chip.addEventListener('click', () => {
  noteInput.value = chip.dataset.vibe;
  vibeChips.forEach((item) => item.classList.toggle('selected', item === chip));
  updatePreview();
}));
...
confirmBtn.addEventListener('click', () => {
  ...
  confirmBtn.textContent = 'Saving our plan…';
  confirmDatePlan();
});
...
startOverBtn.addEventListener('click', () => {
  ...
  confirmBtn.innerHTML = 'Lock in our plan <span aria-hidden="true">💖</span>';
  updatePreview();
});
